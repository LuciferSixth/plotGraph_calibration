...
for interval in intervals:
    ...
    ppg_resampled.columns = ['Timestamp', 'PPG_Max']

    # Resample Sensor
    sensor_resampled = sensor_df_valid.set_index('Timestamp').resample(rule).agg({
        'My sensor': 'max'
    }).reset_index()
    sensor_resampled.columns = ['Timestamp', 'Sensor_Max']

    # Plot Max only
    plt.figure(figsize=(14, 6))
    plt.plot(ppg_resampled['Timestamp'], ppg_resampled['PPG_Max'], 'orange', label='PPG Max', linewidth=2)
    plt.plot(sensor_resampled['Timestamp'], sensor_resampled['Sensor_Max'], 'b', label='Sensor Max', linewidth=2)

    plt.xlabel('Time')
    plt.ylabel('Heart Rate (BPM)')
    plt.title(f'Comparison (Max, Resample {interval} seconds)')
